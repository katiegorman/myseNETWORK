...
(my09<-data.frame(y09$Tree,y09$X,y09$Y,y09$Year))
...
mean(n.relocations09$Freq)
sd(n.relocations09$Freq)
...
rd09<-data.frame(n.roosts09, rs9, names(rs9), rep(2009,length(n.roosts09)))
names(rd09)<-c('NumRoosts','NumLocations','Bat','Year')
...
rum<-glm(NumRoosts~NumLocations+Repro+Year, family=poisson(link='log'), data=arsd)
summary(rum)
Dsquared(rum)
...
V(n09)$color <- c(rep('slateblue',length(vtypen[vtypen=='FALSE'])), rep('yellowgreen',length(vtypen[vtypen=='TRUE'])))
E(n09)$color <- 'black'
...
mean(degd[vtypen=='FALSE']) # bats
sd(degd[vtypen=='FALSE']) # bats

mean(degd[vtypen=='TRUE']) # roosts
sd(degd[vtypen=='TRUE']) # roosts

################################################


# Number of top and bottom nodes
top<-length(V(n09)[type==FALSE]); top
bottom<-length(V(n09)[type==TRUE]); bottom
...
bipartite.density(simplify(n09))
...
# so, FALSE/top == bats
# and TRUE/bottom == roosts
single.mode.betweenness.centralization(n09)
single.mode.closeness.centralization(n09)
#single.mode.degree.centralization(nd) # nonsensical b/c normalization becomes meaningless
single.mode.degree.centralization(simplify(n09)) # simplify removes multi-edges
